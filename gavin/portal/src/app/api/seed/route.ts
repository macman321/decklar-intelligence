...
export async function POST() {
  try {
    ...
    // Seed open items
    const items = [
      { id: "item-1", customer_id: "cust-1", item: "Finalize Q3 device deployment plan", owner: "Jeffrey Calabro", status: "Pending", priority: "high", due_date: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString() },
    ...
    // Seed calls
    const calls = [
      { id: "call-1", customer_id: "cust-1", date: new Date(Date.now() - 3 * 24 * 60 * 60 * 1000).toISOString(), type: "QBR", participants: "Sarah Johnson, Mike Chen", summary: "Reviewed Q2 performance. Customer very satisfied with visibility improvements.", key_decisions: "Expand to 50 more lanes by August" },
      { id: "call-2", customer_id: "cust-1", date: new Date(Date.now() - 18 * 24 * 60 * 60 * 1000).toISOString(), type: "Check-in", participants: "Sarah Johnson", summary: "Routine check-in. No issues raised." },
      { id: "call-3", customer_id: "cust-2", date: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000).toISOString(), type: "Issue Resolution", participants: "David Miller", summary: "Discussed temperature threshold alerts. Adjusted thresholds per customer request." },
      { id: "call-4", customer_id: "cust-4", date: new Date(Date.now() - 2 * 24 * 60 * 60 * 1000).toISOString(), type: "Escalation", participants: "James Wilson", summary: "Escalated connectivity drops in Singapore hub. Engineering team engaged.", key_decisions: "Emergency firmware patch scheduled" },
    ];
